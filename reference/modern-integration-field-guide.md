...
- **Kafka modern shifts (2025-2026):** Kafka 4.0 ships KRaft as the only supported metadata mode (ZooKeeper is removed); new clusters should not deploy ZooKeeper. **KIP-848** (next-gen consumer rebalance protocol, default in 4.0) replaces the stop-the-world rebalance with an incremental cooperative protocol on the broker side; check that your client supports it (`kafka-go` lags here, `franz-go` and confluent clients lead). **KIP-405** tiered storage (GA in OSS Kafka 3.6+, available in MSK and Confluent Cloud) decouples retention from broker disk, making long retention dramatically cheaper. For Go, `github.com/twmb/franz-go` is the modern Kafka-native client with KRaft / KIP-848 support; `segmentio/kafka-go` is in maintenance mode.
